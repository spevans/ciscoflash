...
/*
 * $Id: inode.c,v 1.1 2002-05-21 13:50:35 spse Exp $
 */
...
static void ciscoffs_read_inode(struct inode *i)
{
  ...
  case 0xfffffff0:
    ...
    i->i_mode = S_IFDIR | S_IRWXU | S_IRGRP | S_IXGRP;
    break;
  ...
  default:
    ...
    i->i_mode = S_IFREG | S_IRWXU | S_IRGRP | S_IROTH;   
...
static int ciscoffs_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
  ...
  if(filp->f_pos >= 2) {
    ...
    while(ntohl(fh.magic) == CISCO_FH_MAGIC) {
      DEBUG(1, __FUNCTION__ " :found file %s len = %d\n", 
	    fh.name, ntohl(fh.length));
